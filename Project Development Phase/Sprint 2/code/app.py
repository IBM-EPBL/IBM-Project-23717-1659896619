    ...
    if request.method=="GET":
        ...
        while(row):
            if row[0]==session["email"]:
                ...
                msg="No data found. Add expenses to use."
            row=ibm_db.fetch_both(exec_query)
...
def add_csv():
    if request.method == 'POST':
        try:
            ...
            return render_template('add_csv.html',msg2="Upload only in mentioned format of CSV files!",email=session["email"]) 
